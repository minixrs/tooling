...
#   MINIXRS_LLVM_BASE   default upstream/release/22.x
...
export_fork() { # <name> <dir-basename> <base-ref> <required: 0|1>
    local name="$1" src="$SIBLINGS_DIR/$2" base="$3" required="$4"
    if [ ! -d "$src/.git" ]; then
...
run_one() { # <name> <required>
    case "$1" in
        llvm) export_fork llvm llvm-minixrs "${MINIXRS_LLVM_BASE:-upstream/release/22.x}" "$2" ;;
